...
>
struct ElementType<folly::DynamicBoundedQueue<T, SingleProducer, SingleConsumer, MayBlock, LgSegmentSize, LgAlign, WeightFn, Atom>> {
    using type = T;
};

template<class Queue, size_t Capacity>
struct FollyAdapter : Queue {
    using T = typename ElementType<Queue>::type;
...
void run_throughput_benchmarks(HugePages& hp, std::vector<CpuTopologyInfo> const& cpu_topology) {
    ...
    run_throughput_spsc_benchmark("folly::DSPSCQueue", hp, hw_thread_ids, Type<FollyAdapter<folly::DSPSCQueue<unsigned, false>, SIZE>>{});
    run_throughput_mpmc_benchmark("folly::DMPMCQueue", hp, hw_thread_ids, Type<FollyAdapter<folly::DMPMCQueue<unsigned, false>, SIZE>>{}, 2);
...
void run_ping_pong_benchmarks(HugePages& hp, std::vector<CpuTopologyInfo> const& cpu_topology) {
    ...
    run_ping_pong_benchmark<FollyAdapter<folly::DSPSCQueue<unsigned, false>, SIZE>>("folly::DSPSCQueue", hp, hw_thread_ids);
